...
class AllAuthors extends Component {
  ...
  render() {
    ...
    const MyView = currentData.map((AuList, i) => {
      return (
        <Col key={i.toString()} className="p-0" xl={2} lg={2} md={2} sm={6} xs={6}>
          <Link className = "Link-book" to={'/productauthor/' + AuList.authors_name}>
            <Card className="h-100 w-100 text-center">
              <Card.Body>
                <img className="center" src={AuList.authors_img} alt={AuList.authors_name} />
                <p className="product-name-on-card">{AuList.authors_name}</p>
              </Card.Body>
            </Card>
          </Link>
        </Col>
      ...
      <Fragment>
        ...
        <div className="breadbody">
                              <Breadcrumb>
                                   ...
                                   <Breadcrumb.Item> <Link style={{ color: 'white' }} to="/allauthors"> All Authors</Link> </Breadcrumb.Item>
                              </Breadcrumb>
                         </div>
          <div className="featured-book">
            <h2 style={{ fontWeight: 900 }}>All Authors &nbsp;</h2>
            <p style={{ color: '#1090c3' }}>Explore books with your favorite Authors</p>
          </div>
